#!/usr/bin/env python3
"""Streamlit demo app for exercising the email-processing workflow.

Lets you send customer or forwarder emails within a single thread and inspect
the workflow responses.
"""
...
# Page config
st.set_page_config(
    page_title="SeaRates Logistics AI Sales Assistant", layout="wide", page_icon="🚢"
)
...
with col1:
    st.info(
        f"📧 **Thread ID:** `{st.session_state.thread_id}` - All emails will be part of this thread"
    )
with col2:
...
template_index = 0
if (
    "selected_template_key" in st.session_state
    and st.session_state.selected_template_key != "-- Select a template --"
):
    try:
...
if selected_template != "-- Select a template --" and selected_template in EMAIL_TEMPLATES:
    if (
        "selected_template_key" not in st.session_state
        or st.session_state.selected_template_key != selected_template
    ):
...
with col2:
    if email_type == "Customer":
        sender_email = st.text_input(
            "Customer Email", st.session_state.form_sender_email, key="sender_email_input"
        )
    else:
        sender_email = st.text_input(
            "Forwarder Email", st.session_state.form_sender_email, key="sender_email_input"
        )

subject = st.text_input("Subject", st.session_state.form_subject, key="subject_input")

content = st.text_area(
    "Email Content", st.session_state.form_content, height=250, key="content_textarea"
)
    ...
    if not content.strip():
        ...
        with st.spinner("Processing email through workflow..."):
            try:
                ...
                # Surface the available result keys for visibility
                if not workflow_state:
                    st.warning(
                        "⚠️ No workflow state returned. Result keys: " + str(list(result.keys()))
                    )
                else:
                    available_results = [
                        key for key in workflow_state.keys() if key.endswith("_result")
                    ]
                    if available_results:
                        st.info(
                            f"🔍 Available results in workflow state: {', '.join(available_results)}"
                        )
                ...
                # Record the email in history first so it is captured even if no response is found.
                ...
                # Report when no usable response was found or the response carries an error
                if not response:
                    st.warning(
                        "⚠️ No response found. Checked for: confirmation_response_result, clarification_response_result, confirmation_acknowledgment_result, acknowledgment_response_result"
                    )
                ...
                st.info(
                    f"📝 Email added to history. Total emails: {len(st.session_state.email_history)}"
                )
                ...
                if response and not response.get("error"):
                    ...
                    if body and body != "N/A":
                        ...
                        st.text_area(
                            ...
                            body,
                            height=300,
                            key=f"response_{len(st.session_state.email_history)}",
                    ...
                    with st.expander("🔍 Debug: Why no response?"):
                        ...
                        st.write(
                            list(workflow_state.keys()) if workflow_state else "No workflow_state"
                        )
                ...
                if forwarder_assignment:
                    ...
                    with col1:
                        ...
                        st.markdown(
                            f"- **Route:** {forwarder_assignment.get('origin_country', 'N/A')} → {forwarder_assignment.get('destination_country', 'N/A')}"
                        )
                    ...
                    with col2:
                        if rate_request:
                            ...
                            st.markdown("**Body:**")
                            st.text_area(
                                "Forwarder Email Body",
                                rate_request.get("body", "N/A"),
                                height=200,
                                key=f"forwarder_email_{len(st.session_state.email_history)}",
                                disabled=True,
                                label_visibility="collapsed",
                            )
                ...
                if forwarder_response and not forwarder_response.get("error"):
                    ...
                    if rate_info:
                        ...
                        with col1:
                            ...
                            st.markdown(
                                f"- **Transit Time:** {rate_info.get('transit_time', 'N/A')}"
                            )
                        with col2:
                            st.markdown("**Additional Details:**")
                            st.markdown(f"- **Valid Until:** {rate_info.get('valid_until', 'N/A')}")
                            st.markdown(
                                f"- **Sailing Date:** {rate_info.get('sailing_date', 'N/A')}"
                            )
                ...
                if workflow_state.get("acknowledgment_response_result"):
                    ...
                    if forwarder_ack.get("sender_type") == "forwarder" or email_type == "Forwarder":
                        ...
                        st.markdown("**Body:**")
                        st.text_area(
                            "Forwarder Acknowledgment Body",
                            forwarder_ack.get("body", "N/A"),
                            height=200,
                            key=f"forwarder_ack_{len(st.session_state.email_history)}",
                            disabled=True,
                            label_visibility="collapsed",
                        )
                ...
                if sales_notification and not sales_notification.get("error"):
                    ...
                    st.info(
                        "This email contains customer requirements and forwarder rate information for the sales team."
                    )
                    ...
                    st.text_area(
                        ...
                        sales_notification.get("body", "N/A"),
                        height=400,
                        key=f"sales_notification_{len(st.session_state.email_history)}",
                        disabled=True,
                        label_visibility="collapsed",
                    )
                ...
                if workflow_state.get("customer_quote_result"):
                    ...
                    if not customer_quote.get("error"):
                        ...
                        st.text_area(
                            "Customer Quote Body",
                            customer_quote.get("body", "N/A"),
                            height=400,
                            key=f"customer_quote_{len(st.session_state.email_history)}",
                            disabled=True,
                            label_visibility="collapsed",
                        )
...
if st.session_state.email_history:
    ...
    if last_email.get("forwarder_assignment"):
        ...
        st.info(
            "A forwarder has been assigned. You can respond as the forwarder to test the bot's response."
        )
        ...
        with st.form("forwarder_response_form"):
            ...
            forwarder_content = st.text_area(
                "Forwarder Response",
            ...
            if st.form_submit_button("📤 Send Forwarder Response"):
                with st.spinner("Processing forwarder response..."):
                    try:
                        ...
                        result = asyncio.run(
                            st.session_state.orchestrator.process_email(email_data)
                        )
                        workflow_state = result.get("result", {})
...
if st.session_state.email_history:
    ...
    for idx, email in enumerate(reversed(st.session_state.email_history)):
        ...
        with st.expander(
            f"📧 Email #{email_num}: {email_type} - {subject} ({timestamp})", expanded=(idx == 0)
        ):
            # Email metadata

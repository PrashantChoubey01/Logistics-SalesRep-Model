# !/usr/bin/env python3
"""
LangGraph workflow orchestrator for the multi-LLM logistics CRM system.

Wires the agents together with thread management and forwarder handling.
"""
...
# Import utilities
from utils.thread_manager import EmailEntry, ThreadManager

logger = get_logger(__name__)


def _escalation_reducer(
    x: Optional[Dict[str, Any]], y: Optional[Dict[str, Any]]
) -> Optional[Dict[str, Any]]:
    """Take the first non-None escalation_result to avoid concurrent-update conflicts."""
    return x if x is not None else y


def _should_escalate_reducer(x: bool, y: bool) -> bool:
    """Combine concurrent should_escalate flags with OR semantics."""
    return x or y


def _sales_notification_reducer(
    x: Optional[Dict[str, Any]], y: Optional[Dict[str, Any]]
) -> Optional[Dict[str, Any]]:
    """Take the first non-None sales_notification_result to avoid conflicts."""
    return x if x is not None else y


def _forwarder_response_reducer(
    x: Optional[Dict[str, Any]], y: Optional[Dict[str, Any]]
) -> Optional[Dict[str, Any]]:
    """Take the first non-None forwarder_response_result so only one node sets it per step."""
...
class LangGraphWorkflowOrchestrator:
    ...
    def _initialize_agents(self):
        ...
        try:
            ...
            agents = [
                ...
                self.conversation_state_agent,
                self.thread_analysis_agent,
                self.extraction_agent,
                self.validation_agent,
                self.port_lookup_agent,
                self.container_standardization_agent,
                self.rate_recommendation_agent,
                self.next_action_agent,
                self.clarification_agent,
                self.confirmation_agent,
                self.acknowledgment_agent,
                self.confirmation_acknowledgment_agent,
                self.forwarder_detection_agent,
                self.forwarder_response_agent,
                self.forwarder_email_draft_agent,
                self.escalation_agent,
                self.sales_notification_agent,
            ]
    ...
    def _build_workflow_graph(self):
        ...
        workflow.add_node(
            "generate_acknowledgment_response", self._generate_acknowledgment_response
        )
        workflow.add_node(
            "generate_confirmation_acknowledgment", self._generate_confirmation_acknowledgment
        )
        ...
        workflow.add_conditional_edges(
            ...
            {"assign_forwarders": "assign_forwarders", "update_thread": "update_thread"},
        )

        # Forwarder assignment leads to thread update
        workflow.add_edge("assign_forwarders", "update_thread")

        # Acknowledgment response routing
        ...
        workflow.add_edge(
            "process_forwarder_response", "notify_sales"
        )  # After processing forwarder response, notify sales
    ...
    async def _classify_email(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            logger.info(
                f" Email classified as: {result.get('email_type', 'unknown')} (confidence: {result.get('confidence', 0.0)})"
            )
    ...
    async def _conversation_state(self, state: WorkflowState) -> WorkflowState:
        ...
        print(f"\n{'='*60}")
        print("STEP 2: CONVERSATION STATE ANALYSIS")
        print(f"{'='*60}")
        ...
        try:
            ...
            content = email_data.get(
                "content", email_data.get("body_text", email_data.get("body", ""))
            )
            subject = email_data.get("subject", "")
            ...
            logger.info(
                f" Conversation state analyzed: {result.get('conversation_stage', 'unknown')}"
            )
    ...
    async def _analyze_thread(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            # Handle both content/body_text and sender/from_email formats
            email_content = email_data_raw.get(
                "content", email_data_raw.get("body_text", email_data_raw.get("body", ""))
            )
            email_subject = email_data_raw.get("subject", "")
            email_sender = email_data_raw.get(
                "sender", email_data_raw.get("from_email", email_data_raw.get("from", ""))
            )
            ...
            for email_entry in state["thread_history"]:
                if (
                    "step_results" in email_entry
                    and "classification" in email_entry["step_results"]
                ):
            ...
            logger.info(
                f" Thread analysis completed: {len(result.get('key_insights', []))} insights found"
            )
    ...
    async def _extract_information(self, state: WorkflowState) -> WorkflowState:
        ...
        print(f"\n{'='*60}")
        print("STEP 4: INFORMATION EXTRACTION")
        print(f"{'='*60}")
        email_data_raw = state.get("email_data", {})
        email_content = email_data_raw.get(
            "content", email_data_raw.get("body_text", email_data_raw.get("body", ""))
        ...
        try:
            ...
            logger.info(
                f" Information extraction completed: {len(extracted_data)} categories extracted"
            )
    ...
    async def _update_cumulative_extraction(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if new_extraction.get("shipment_details"):
                ...
                logger.info(
                    f" DEBUG: New extraction - Origin={new_shipment.get('origin')}, Destination={new_shipment.get('destination')}, Container={new_shipment.get('container_type')}, Commodity={new_shipment.get('commodity')}"
                )

            if state.get("cumulative_extraction") and state["cumulative_extraction"].get(
                "shipment_details"
            ):
                old_shipment = state["cumulative_extraction"]["shipment_details"]
                logger.info(
                    f" DEBUG: Old cumulative - Origin={old_shipment.get('origin')}, Destination={old_shipment.get('destination')}, Container={old_shipment.get('container_type')}, Commodity={old_shipment.get('commodity')}"
                )
            ...
            if new_extraction:
                ...
                updated_cumulative = self.thread_manager.merge_with_recency_priority(
                    new_extraction, state["cumulative_extraction"]
                )

                if updated_cumulative.get("shipment_details"):
                    merged_shipment = updated_cumulative["shipment_details"]
                    logger.info(
                        f" DEBUG: Merged result - Origin={merged_shipment.get('origin')}, Destination={merged_shipment.get('destination')}, Container={merged_shipment.get('container_type')}, Commodity={merged_shipment.get('commodity')}"
                    )
                ...
                self.thread_manager.update_cumulative_extraction(
                    state["thread_id"], updated_cumulative
                )

                logger.info(
                    f" Cumulative extraction updated with {len(new_extraction)} new categories"
                )
                ...
                logger.warning(" No new extraction data to merge")
    ...
    async def _lookup_ports(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if state["cumulative_extraction"] and state["cumulative_extraction"].get(
                "shipment_details"
            ):
                ...
                shipment_details = (
                    state["extraction_result"].get("extracted_data", {}).get("shipment_details", {})
                )
            ...
            origin_result = None
            if original_origin:
                origin_result = self.port_lookup_agent.process({"port_name": original_origin})
                ...
                if origin_result:
                    ...
                    if (
                        origin_country
                        and port_country
                        and origin_country.upper() != port_country.upper()
                    ):
                        logger.warning(
                            f" COUNTRY MISMATCH: origin_country='{origin_country}' but port '{original_origin}' is in '{port_country}' - nullifying origin_country"
                        )
                        shipment_details["origin_country"] = ""
                        if state["cumulative_extraction"]:
                            state["cumulative_extraction"]["shipment_details"][
                                "origin_country"
                            ] = ""
                        logger.info(" Nullified origin_country due to mismatch")
                    ...
                    if not port_code or confidence < 0.6:
                        logger.warning(
                            f" Port code not found or low confidence ({confidence:.2f}) for origin '{original_origin}' - preserving port name"
                        )
                        ...
                        # Set port_name to original if it's None (port lookup failed)
                        if (
                            not origin_result.get("port_name")
                            or origin_result.get("port_name") is None
                        ):
            ...
            if original_destination:
                ...
                if destination_result:
                    ...
                    destination_country = (
                        shipment_details.get("destination_country") or ""
                    ).strip()
                    ...
                    if (
                        destination_country
                        and port_country
                        and destination_country.upper() != port_country.upper()
                    ):
                        logger.warning(
                            f" COUNTRY MISMATCH: destination_country='{destination_country}' but port '{original_destination}' is in '{port_country}' - nullifying destination_country"
                        )
                        ...
                        logger.info(" Nullified destination_country due to mismatch")
                    ...
                    if not port_code or confidence < 0.6:
                        logger.warning(
                            f" Port code not found or low confidence ({confidence:.2f}) for destination '{original_destination}' - preserving port name"
                        )
                        # Ensure port name is preserved in cumulative extraction
                        if state["cumulative_extraction"]:
                            state["cumulative_extraction"]["shipment_details"][
                        ...
                        # Set port_name to original if it's None (port lookup failed)
                        if (
                            not destination_result.get("port_name")
                            or destination_result.get("port_name") is None
                        ):
                            destination_result["port_name"] = original_destination

            # Combine results
            result = {"origin": origin_result, "destination": destination_result, "port_codes": {}}

            # Extract port codes ONLY if confidence >= 0.6 and port_code exists
            if (
                origin_result
                and origin_result.get("port_code")
                and origin_result.get("confidence", 0.0) >= 0.6
            ):
                result["port_codes"]["origin"] = origin_result["port_code"]
            elif origin_result and (
                not origin_result.get("port_code") or origin_result.get("confidence", 0.0) < 0.6
            ):
                logger.info(
                    f" Origin port code not added to port_codes (confidence: {origin_result.get('confidence', 0.0):.2f})"
                )

            if (
                destination_result
                and destination_result.get("port_code")
                and destination_result.get("confidence", 0.0) >= 0.6
            ):
                result["port_codes"]["destination"] = destination_result["port_code"]
            elif destination_result and (
                not destination_result.get("port_code")
                or destination_result.get("confidence", 0.0) < 0.6
            ):
                logger.info(
                    f" Destination port code not added to port_codes (confidence: {destination_result.get('confidence', 0.0):.2f})"
                )

            state["port_lookup_result"] = result
            logger.info(
                f" Port lookup completed: {len(result.get('port_codes', {}))} port codes found"
            )
    ...
    async def _standardize_container(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            # Check shipment_type - skip container standardization for LCL shipments
            shipment_type = (
                shipment_details.get("shipment_type", "").strip().upper()
                if shipment_details.get("shipment_type")
                else ""
            )

            if shipment_type == "LCL":
                # LCL shipments don't need container standardization
                logger.info(" Skipping container standardization for LCL shipment")
            ...
            container_type = (
                shipment_details.get("container_type", "").strip()
                if shipment_details.get("container_type")
                else ""
            )
            if not container_type:
                logger.info(" No container type to standardize")
                state["container_standardization_result"] = {
                    "standardized_type": None,
            ...
            logger.info(
                f" Container standardization completed: {result.get('standardized_type', 'unknown')}"
            )
    ...
    async def _recommend_rates(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if not is_fcl:
                logger.info(" Skipping rate recommendation for LCL shipment")
            ...
            enhanced_shipment_details["container_type"] = (
                container_type  # Use standardized container type
            )

            # Log the data being sent to rate recommendation
            logger.info(" Rate Recommendation Input Data:")
            ...
            logger.info(
                f" Rate recommendation completed: {len(result.get('rate_ranges', {}))} routes analyzed"
            )
    ...
    async def _next_action(self, state: WorkflowState) -> WorkflowState:
        ...
        print(f"\n{'='*60}")
        print("STEP 10: NEXT ACTION DETERMINATION")
        print(f"{'='*60}")
        print(f"Classification: {state['classification_result'].get('email_type', 'unknown')}")
        print(
            f"Conversation State: {state['conversation_state_result'].get('conversation_stage', 'unknown')}"
        )
        print(f"Extraction Quality: {state['extraction_result'].get('quality_score', 0.0)}")
        print(
            f"Validation Status: {state['validation_result'].get('validation_status', 'unknown')}"
        )
        ...
        try:
            ...
            result = self.next_action_agent.process(
                {
                    "conversation_state": state["conversation_state_result"].get(
                    ...
                    "enriched_data": {
                        ...
                        "container_standardization": state.get(
            ...
            logger.info(
                f" Next action determined: {result.get('next_action', result.get('action', 'unknown'))}"
            )
    ...
    async def _assign_sales_person(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if not extraction_result:
                ...
                logger.info(" No extraction result available, using default route info")
                route_info = {
                    "origin": "",
                    "destination": "",
                    "container_type": "",
    ...
    async def _generate_clarification_response(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            # Reload cumulative_extraction from thread_manager to ensure we have the latest merged data
            ...
            if thread_id:
                ...
                if latest_cumulative and isinstance(latest_cumulative, dict):
                    ...
                    logger.info(
                        " Using latest cumulative extraction from thread manager for clarification response"
                    )
                elif state.get("cumulative_extraction") and isinstance(
                    state["cumulative_extraction"], dict
                ):
                    extracted_data = copy.deepcopy(state["cumulative_extraction"])
                    logger.info(
                        " Using cumulative extraction from state for clarification response"
                    )
                else:
                    extracted_data = state["extraction_result"].get("extracted_data", {})
                    logger.info(
                        " Using current extraction data for clarification response (no cumulative data available)"
                    )
            elif state.get("cumulative_extraction") and isinstance(
                state["cumulative_extraction"], dict
            ):
            ...
            logger.info(
                f" DEBUG: Container standardization result: {state.get('container_standardization_result')}"
            )
            ...
            # Only add container_type if shipment_type is NOT LCL
            shipment_type = (
                extracted_data.get("shipment_details", {}).get("shipment_type", "").strip().upper()
                if extracted_data.get("shipment_details", {}).get("shipment_type")
                else ""
            )
            ...
            if shipment_type == "LCL":
                ...
                logger.info(
                    " DEBUG: LCL shipment detected - clearing container_type and container_count"
                )
                if "shipment_details" in extracted_data:
            ...
            elif state.get("container_standardization_result") and isinstance(
                state["container_standardization_result"], dict
            ):
                # Get standardized_type (the actual standardized type, e.g., "40HC")
                standardized_type = state["container_standardization_result"].get(
                    "standardized_type"
                )
                ...
                logger.info(
                    f" DEBUG: Rate fallback type (NOT used for display): {state['container_standardization_result'].get('rate_fallback_type')}"
                )
                ...
                if standardized_type:
                    ...
                    if "shipment_details" in extracted_data:
                        ...
                        logger.info(
                            f" DEBUG: Updated container type from '{original_type}' to '{standardized_type}'"
                        )
                    else:
                        ...
                        extracted_data["shipment_details"]["container_type"] = standardized_type
                        logger.info(
                            f" DEBUG: Created shipment_details and set container type to '{standardized_type}'"
                        )
                else:
                    logger.warning(" DEBUG: Standardized type is empty/None")
            else:
                logger.warning(" DEBUG: No container standardization result available")

            result = self.clarification_agent.process(
                {
                    "extracted_data": extracted_data,
                    "missing_fields": missing_fields,  # Empty list - agent will determine missing fields
                    "customer_name": state["email_data"].get(
                        "sender_name", "Valued Customer"
                    ),  # First name extracted from email
                    "agent_info": state["assigned_sales_person"],
                    "port_lookup_result": state[
                        "port_lookup_result"
                    ],  # For enriched port display with codes
                    "container_standardization_result": state.get(
                        "container_standardization_result"
                    ),  # For reference
                }
            )
    ...
    def _validate_mandatory_fields_for_confirmation(
        self, extracted_data: Dict[str, Any], port_lookup_result: Dict[str, Any] = None
    ) -> tuple:
        """
        ...
        # Check extracted shipment_type FIRST (directly extracted from email)
        ...
        if isinstance(container_type_raw, dict):
            container_type = (
                container_type_raw.get("standardized_type", "")
                or container_type_raw.get("standard_type", "")
                or container_type_raw.get("original_input", "")
            )
        ...
        # Do NOT assume shipment type - ask for it if missing
        ...
        if shipment_type == "LCL":
            ...
            if is_explicitly_lcl:
                ...
                is_fcl = True
            # If shipment_type is not explicitly mentioned, do NOT assume - ask for it
        ...
        shipment_date = (
            shipment_details.get("shipment_date", "").strip()
            or timeline_info.get("requested_dates", "").strip()
            or timeline_info.get("shipment_date", "").strip()
        )

        # If shipment_type is not explicitly mentioned, ask for ALL required fields
        if is_fcl is None:
        ...
        if is_fcl:
            ...
            # Container count IS required for FCL shipments
            quantity = (
                shipment_details.get("container_count", "").strip()
                or shipment_details.get("quantity", "").strip()
            )
            if not quantity:
            ...
            # Container count is NEVER required for LCL shipments - this is a hard rule
            # Remove container_count if it was somehow added
            # This ensures LCL shipments NEVER ask for container_count
            missing_fields = [
                f
                for f in missing_fields
                if "container_count" not in f.lower()
                and "number of containers" not in f.lower()
                and "quantity (number of containers)" not in f.lower()
            ]
    ...
    async def _generate_confirmation_response(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if state.get("cumulative_extraction") and isinstance(
                state["cumulative_extraction"], dict
            ):
                extracted_data = copy.deepcopy(state["cumulative_extraction"])
                logger.info(
                    " Using cumulative extraction from state for confirmation response (most recent)"
                )
            # PRIORITY 2: Use current extraction result (if cumulative not available)
            elif state.get("extraction_result") and state["extraction_result"].get(
                "extracted_data"
            ):
            ...
            # PRIORITY 3: Fallback to thread_manager (may be stale, but better than nothing)
                ...
                if thread_id:
                    ...
                    if latest_cumulative and isinstance(latest_cumulative, dict):
                        ...
                        extracted_data = {}
                        logger.warning(" No extraction data available for confirmation response")
                else:
                    extracted_data = {}
                    logger.warning(" No thread_id and no extraction data available")

            logger.info(
                f" DEBUG: Confirmation response using extracted_data keys: {list(extracted_data.keys())}"
            )
            if extracted_data.get("shipment_details"):
                shipment = extracted_data["shipment_details"]
                logger.info(
                    f" DEBUG: Origin={shipment.get('origin')}, Destination={shipment.get('destination')}, Container={shipment.get('container_type')}, Commodity={shipment.get('commodity')}"
                )
            ...
            if not is_valid:
                logger.error(
                    f" Cannot generate confirmation - mandatory fields missing: {missing_fields}"
                )
                logger.warning(" Overriding to clarification due to missing mandatory fields")
            ...
            if (
                shipment_type != "LCL"
                and state.get("container_standardization_result")
                and isinstance(state["container_standardization_result"], dict)
            ):
                standardized_type = state["container_standardization_result"].get(
                    "standardized_type"
                )
            ...
            result = self.confirmation_agent.process(
                {
                    ...
                    "port_lookup_result": state.get(
                        "port_lookup_result"
                    ),  # For human-friendly port formatting (per spec)
                }
            )
            ...
            # Print final response on terminal
            print(f"\n{'='*80}")
            print("FINAL RESPONSE GENERATED: CONFIRMATION")
            print(f"{'='*80}")
            print(f"Subject: {result.get('subject', 'No subject')}")
            print(f"To: {state['email_data'].get('sender', 'Unknown')}")
            print("Response Type: Confirmation Request")
            print("\nRESPONSE BODY:")
            print(f"{result.get('body', 'No body')}")
            print(f"{'='*80}")
    ...
    async def _generate_acknowledgment_response(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if not assigned_sales_person:
                logger.info(" No sales person assigned, assigning one for acknowledgment")
                ...
                try:
                    ...
                    assigned_sales_person = self.sales_team_manager.assign_sales_person(
                        route_info, {}
                    )
                    state["assigned_sales_person"] = assigned_sales_person
                    logger.info(
                        f" Sales person assigned for acknowledgment: {assigned_sales_person.get('name', 'Unknown')}"
                    )
            ...
            print(
                f"From: {assigned_sales_person.get('name', 'Unknown')} ({assigned_sales_person.get('email', 'Unknown')})"
            )
            print("\nRESPONSE BODY:")
            print(f"{result.get('body', 'No body')}")
            print(f"{'='*80}")
            ...
            logger.info(
                f" {sender_type.title()} acknowledgment response generated: {result.get('subject', 'No subject')}"
            )

            # Return only the fields we modify (don't include email_data)
            return {"acknowledgment_response_result": result}

        except Exception as e:
            logger.error(f" Acknowledgment response generation failed: {e}")
            # Return only the fields we modify
            return {"acknowledgment_response_result": {"error": str(e)}}
        ...
        try:
            ...
            if thread_id:
                ...
                if latest_cumulative and isinstance(latest_cumulative, dict):
                    extracted_data = copy.deepcopy(latest_cumulative)
                    logger.info(
                        " Using latest cumulative extraction from thread manager for confirmation acknowledgment"
                    )
                elif state.get("cumulative_extraction") and isinstance(
                    state["cumulative_extraction"], dict
                ):
                    extracted_data = copy.deepcopy(state["cumulative_extraction"])
                    logger.info(
                        " Using cumulative extraction from state for confirmation acknowledgment"
                    )
                else:
                    extraction_result = state.get("extraction_result", {})
                    extracted_data = (
                        extraction_result.get("extracted_data", {}) if extraction_result else {}
                    )
                    logger.info(
                        " Using current extraction data for confirmation acknowledgment (no cumulative data available)"
                    )
            elif state.get("cumulative_extraction") and isinstance(
                state["cumulative_extraction"], dict
            ):
            ...
            # Validate Priority 1-3 mandatory fields before generating confirmation acknowledgment
            ...
            if not is_valid:
                logger.error(
                    f" Cannot generate confirmation acknowledgment - mandatory fields missing: {missing_fields}"
                )
                logger.warning(
            ...
            email_data = state.get("email_data", {})
            customer_name = (
                email_data.get("sender_name", "Valued Customer")
                if email_data
            ...
            # Print final response on terminal
            print(f"\n{'='*80}")
            print("FINAL RESPONSE GENERATED: CONFIRMATION ACKNOWLEDGMENT")
            print(f"{'='*80}")
            print(f"Subject: {result.get('subject', 'No subject')}")
            print(f"To: {email_data.get('sender', 'Unknown')}")
            print("Response Type: Confirmation Acknowledgment")
            print("\nRESPONSE BODY:")
            print(f"{result.get('body', 'No body')}")
            print(f"{'='*80}")

            logger.info(
                f" Confirmation acknowledgment response generated: {result.get('subject', 'No subject')}"
            )
    ...
    async def _detect_forwarder(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            result = self.forwarder_detection_agent.process(
                {"email_data": email_data, "forwarder_manager": self.forwarder_manager}
            )

            state["forwarder_detection_result"] = result
            state["is_forwarder_email"] = result.get("is_forwarder", False)
            logger.info(
                f" Forwarder detection completed: {result.get('forwarder_name', 'Unknown')}"
            )
    ...
    async def _process_forwarder_response(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            return {"forwarder_response_result": {"error": str(e)}}
    ...
    def _resolve_route_countries(self, shipment_details: dict, port_lookup_result: dict) -> tuple:
        ...
        return (
            origin if self._is_valid_country(origin) else "",
    ...
    async def _assign_forwarders(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            assigned_forwarder, assignment_reason = (
                self.forwarder_manager.assign_forwarder_with_reason(
            ...
            if assigned_forwarder:
                ...
                email_data = state.get("email_data", {})
                customer_email_content = email_data.get(
                    "content", email_data.get("body_text", email_data.get("body", ""))
                )
                ...
                if state.get("container_standardization_result") and isinstance(
                    state["container_standardization_result"], dict
                ):
                    standardized_type = state["container_standardization_result"].get(
                        "standardized_type"
                ...
                if cumulative_extraction and cumulative_extraction.get("timeline_information"):
                    ...
                    if not complete_shipment_details.get("shipment_date") and timeline_info.get(
                        "requested_dates"
                    ):
                        complete_shipment_details["shipment_date"] = timeline_info.get(
                            "requested_dates"
                        )
                        logger.info(
                            f" Added shipment_date from timeline_information: {timeline_info.get('requested_dates')}"
                        )
                ...
                if rate_request_email:
                    ...
                    print(
                        f"From: {state.get('assigned_sales_person', {}).get('email', 'sales@logistics-company.com')}"
                    )
                    print("\nBody:")
                ...
                logger.info(
                    f" Rate request generated for {origin_country} to {destination_country}"
                )
            else:
                state["forwarder_assignment_result"] = {
                ...
                logger.warning(
                    f" No forwarder available for route: {origin_country} to {destination_country}"
                )

        except Exception as e:
            logger.error(f" Forwarder assignment failed: {e}")
            state["forwarder_assignment_result"] = {"error": str(e), "status": "error"}
    ...
    async def _check_escalation(self, state: WorkflowState) -> WorkflowState:
        ...
        if state.get("escalation_result") is not None:
            logger.info(" Escalation already processed, skipping duplicate call")
        ...
        try:
            ...
            email_sender = email_data.get(
                "sender", email_data.get("from_email", email_data.get("from", ""))
            )
            ...
            # Print final response on terminal
            print(f"\n{'='*80}")
            print("FINAL RESPONSE GENERATED: ESCALATION")
            print(f"{'='*80}")
            print(f"Subject: {result.get('subject', 'Escalation Required')}")
            print("To: Sales Team")
            print("Response Type: Escalation")
            ...
            logger.info(
                f" Escalation check completed: {'Escalate' if result.get('should_escalate', False) else 'Continue'}"
            )
    ...
    async def _notify_sales(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if forwarder_response_result and not forwarder_response_result.get("error"):
                ...
                rate_info = forwarder_response_result.get(
                    "extracted_rate_info", {}
...
"""
                ...
                forwarder_details = {
                    ...
                    "company": forwarder_response_result.get(
                        "forwarder_name", ""
                    ),  # Use name as company fallback
                }
            ...
            if forwarder_detection_result and not forwarder_detection_result.get("error"):
                forwarder_detection_details = forwarder_detection_result.get(
                    "forwarder_details", {}
                )
                if forwarder_detection_details:
                    if (
                        not forwarder_details.get("name")
                        or forwarder_details.get("name") == "Forwarder"
                    ):
                    ...
                    if not forwarder_details.get("company"):
                        forwarder_details["company"] = forwarder_detection_details.get(
                            "company", forwarder_detection_details.get("name", "")
                        )
            ...
            if not forwarder_details.get("name") or forwarder_details.get("name") == "Forwarder":
                ...
                if sender_email:
                    ...
                    forwarder_details["name"] = self._extract_name_from_email(
                        sender_email, forwarder_email_content
                    )
                    if not forwarder_details.get("email"):
...
"""
            ...
            # Print final response on terminal
            print(f"\n{'='*80}")
            print("FINAL RESPONSE GENERATED: SALES NOTIFICATION")
            print(f"{'='*80}")
            print(f"Subject: {result.get('subject', 'Sales Notification')}")
            print("To: Sales Team")
            print("Response Type: Sales Notification")
            ...
            # Return only the fields we modify (don't include email_data)
            return {"sales_notification_result": result}

        except Exception as e:
            logger.error(f" Sales notification failed: {e}")
            # Return only the fields we modify
            return {"sales_notification_result": {"error": str(e)}}
    ...
    async def _generate_customer_quote(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            shipment_details = (
                cumulative_extraction.get("shipment_details", {}) if cumulative_extraction else {}
            )

            # Customer identity comes from the thread's contact information, NOT the
            # forwarder email that triggered this quote.
            contact_info = (
                cumulative_extraction.get("contact_information", {})
            ...
            if not customer_email:
                ...
                fwd_email = (
                    ((state.get("forwarder_response_result") or {}).get("forwarder_email") or "")
                    .strip()
                    .lower()
                )
...
"""
            ...
            headline_rate = (
                rate_info.get("rate") or rate_info.get("rates_with_dthc") if rate_info else None
            )
...
"""
            ...
            # Return only the field we modify
            return {"customer_quote_result": customer_quote_result}

        except Exception as e:
            logger.error(f" Customer quote generation failed: {e}")
            return {"customer_quote_result": {"error": str(e)}}
    ...
    async def _update_thread(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            email_sender = email_data.get(
                "sender", email_data.get("from_email", email_data.get("from", "unknown"))
            )
            ...
            customer_email_entry = EmailEntry(
                ...
                response_type=(
                    next_action_result.get("action", "unknown") if next_action_result else "unknown"
                ),
            ...
            try:
                ...
                if not thread_data:
                    logger.warning(" Thread manager returned None, creating fallback thread data")
                    # Create a fallback thread data structure
                    thread_data = type(
                        "ThreadData", (), {"cumulative_extraction": {}, "total_emails": 1}
                    )()
            ...
            if response_result:
                ...
                try:
                    ...
                    if not thread_data:
                        logger.warning(
                            " Thread manager returned None for bot response, using existing thread data"
                        )
            ...
            if thread_data and hasattr(thread_data, "cumulative_extraction"):
                ...
                logger.warning(" Thread data not available, using empty cumulative extraction")
                state["cumulative_extraction"] = {}

            # Ensure thread_data is always defined for safety
            if not thread_data:
                logger.warning(" Creating fallback thread data")
                thread_data = type(
                    "ThreadData", (), {"cumulative_extraction": {}, "total_emails": 1}
                )()
    ...
    def _route_after_classification(self, state: WorkflowState) -> str:
        ...
        print(
            f"ROUTING: Email type = {email_type}, Sender type = {sender_type}, Sender Classification = {sender_classification_type}"
        )

        # Route based on sender classification first (most reliable)
        if sender_classification_type == "sales_person":
            print("Sales person email detected - routing to acknowledgment")
            return "generate_acknowledgment_response"
        elif sender_classification_type == "forwarder":
            print("Forwarder email detected - routing to acknowledgment")
            return "generate_acknowledgment_response"
        elif (
            "customer" in email_type
            or sender_type == "customer"
            or sender_classification_type == "customer"
        ):
            ...
            print("Default routing to conversation_state (happy flow)")
            return "conversation_state"
    ...
    def _route_after_next_action(self, state: WorkflowState) -> str:
        ...
        next_action_result = state.get("next_action_result", {})
        action = next_action_result.get(
            "next_action", next_action_result.get("action", "send_confirmation_request")
        )
        ...
        if action in [
            ...
            print("Default routing to assign_sales_person (happy flow)")
            return "assign_sales_person"
    ...
    def _route_after_sales_assignment(self, state: WorkflowState) -> str:
        ...
        # Validate mandatory fields directly in routing to ensure accuracy
        ...
        overall_confidence = (
            classification_confidence + extraction_confidence + validation_confidence
        ) / 3

        # Check for customer confirmation
        conversation_state = state.get("conversation_state_result", {}).get(
            "conversation_state", ""
        )
        ...
        customer_confirmed = (
            "confirmation" in conversation_state.lower()
            or "confirmed" in conversation_state.lower()
            or email_type == "customer_confirmation"
            or "confirmation" in email_type
            or "confirmed" in email_type
        )
        ...
        if missing_fields and len(missing_fields) > 0:
            ...
            print("   PRIORITY 1: Sending clarification request")
        ...
        if not customer_confirmed:
            print("   NO MISSING FIELDS: All information is complete")
            print("   PRIORITY 2: Sending confirmation request (customer not yet confirmed)")
        ...
        if customer_confirmed:
            print("   CUSTOMER CONFIRMATION DETECTED + ALL FIELDS COMPLETE")
            print("   PRIORITY 3: Sending confirmation acknowledgment")
        ...
        if overall_confidence < LOW_CONFIDENCE_THRESHOLD:
            print(
                f"   LOW CONFIDENCE DETECTED: {overall_confidence:.2f} < {LOW_CONFIDENCE_THRESHOLD}"
            )
            print("   FALLBACK: Sending clarification request")
            return "generate_clarification_response"

        # Final fallback: clarification
        print("   FINAL FALLBACK: Sending clarification request")
        return "generate_clarification_response"

    def _route_after_acknowledgment(self, state: WorkflowState) -> str:
        """Route after acknowledgment response."""
        ...
        if sender_type == "forwarder" or email_type == "forwarder_response":
            print("Forwarder email detected - processing response and notifying sales")
            # Process forwarder response first, then notify sales
            return "process_forwarder_response"

        if sender_type == "sales_person":
            print("Sales person email - proceeding to thread update")
            return "update_thread"

        print("ACKNOWLEDGMENT ROUTING: Proceeding to thread update (happy flow)")
        return "update_thread"
    ...
    def _extract_name_from_email(self, sender_email: str, email_content: str = "") -> str:
        ...
        if sender_email and "@" in sender_email:
            ...
            email_prefix = re.sub(
                r"^(info|contact|sales|quotes|rates|support|hello|hi)",
                "",
                email_prefix,
                flags=re.IGNORECASE,
            )
    ...
    def _route_after_confirmation_acknowledgment(self, state: WorkflowState) -> str:
        ...
        # If clarification was generated (because confirmation acknowledgment failed),
        # do NOT assign forwarders - just update thread
        if clarification_result and not clarification_result.get("error"):
            print(
                "CONFIRMATION ACKNOWLEDGMENT ROUTING: Clarification was generated instead, skipping forwarder assignment"
            )
            return "update_thread"
        ...
        if confirmation_ack_result and not confirmation_ack_result.get("error"):
            print("CONFIRMATION ACKNOWLEDGMENT ROUTING: Proceeding with forwarder assignment")
            ...
            print(
                "CONFIRMATION ACKNOWLEDGMENT ROUTING: Error detected, skipping forwarder assignment (clarification was generated)"
            )
            return "update_thread"
    ...
    async def process_email(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if normalized_email_data:
            ...
            if "content" not in normalized_email_data:
                normalized_email_data["content"] = normalized_email_data.get(
                    "body_text", normalized_email_data.get("body", "")
                )
            # Ensure sender exists (use from_email as fallback)
            if "sender" not in normalized_email_data:
                normalized_email_data["sender"] = normalized_email_data.get(
                    "from_email", normalized_email_data.get("from", "unknown")
                )
        ...
        sender_email = normalized_email_data.get(
            "sender", normalized_email_data.get("from_email", "unknown")
        )
        ...
        try:
            ...
            if existing_thread:
                try:
                        ...
                        for email in existing_thread.email_chain:
                            if email is not None:
                                try:
                                    ...
                                    logger.warning(
                                        f" Failed to convert email to dict: {email_error}"
                                    )
                    ...
                    initial_state["cumulative_extraction"] = getattr(
                        existing_thread, "cumulative_extraction", {}
                    )
                    initial_state["customer_context"] = getattr(
                        existing_thread, "customer_context", {}
                    )
                    initial_state["forwarder_context"] = getattr(
                        existing_thread, "forwarder_context", {}
                    )
                    logger.info(
                        f" Loaded existing thread with {len(initial_state['thread_history'])} emails"
                    )
            ...
            try:
                ...
                if initial_state.get("acknowledgment_response_result"):
                    partial_result["acknowledgment_response_result"] = initial_state[
...
John Smith""",
    ...
    try:
        result = await orchestrator.process_email(
            ...
            customer_context={
                "priority": "high",
                "customer_info": {"name": "John Smith", "company": "Tech Solutions Inc."},
            },
            forwarder_context={},
            market_data={},
            historical_data=[],

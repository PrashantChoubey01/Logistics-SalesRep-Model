...
export DATABRICKS_BASE_URL="${DATABRICKS_BASE_URL:-https://adb-1825279086009288.8.azuredatabricks.net/serving-endpoints}"
export MODEL_ENDPOINT_ID="${MODEL_ENDPOINT_ID:-databricks-claude-3-7-sonnet}"
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        echo "   ⚠️  API server health check timeout (may still be starting)"
    fi
    sleep 1
done
...
for i in {1..10}; do
    ...
    if [ $i -eq 10 ]; then
        echo "   ⚠️  Frontend server health check timeout (may still be starting)"
    fi
    sleep 1
done

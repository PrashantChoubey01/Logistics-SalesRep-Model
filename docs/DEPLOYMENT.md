...
2. **Databricks API Token**
   - You need a valid Databricks token to access the Claude Sonnet model
...
DATABRICKS_BASE_URL=https://adb-2252852771922438.18.azuredatabricks.net/serving-endpoints/
MODEL_ENDPOINT_ID=databricks-claude-sonnet-4-6
```
...
docker run -d \
  ...
  -e DATABRICKS_BASE_URL=https://adb-2252852771922438.18.azuredatabricks.net/serving-endpoints/ \
  -e MODEL_ENDPOINT_ID=databricks-claude-sonnet-4-6 \
...
| `MODEL_ENDPOINT_ID` | No | `databricks-claude-sonnet-4-6` | Model endpoint name |
...
Your Logistics AI Bot is now containerized and ready to run anywhere Docker is available.

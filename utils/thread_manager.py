#!/usr/bin/env python3
"""Thread manager for multi-party email conversations.
...
class ThreadManager:
    ...
    def __init__(self, storage_dir: str = "data/threads"):
        ...
        # Merge strategy per data category
    ...
    def _merge_shipment_details(
        self, new_data: Dict[str, Any], cumulative_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        logger.debug(
            f"Merge shipment_details - Cumulative keys: {list(cumulative_data.keys()) if cumulative_data else []}, New keys: {list(new_data.keys())}"
        )

        # Handle an explicit shipment_type change, clearing fields specific to the other mode.
        new_shipment_type = (
            new_data.get("shipment_type", "").strip().upper()
            if new_data.get("shipment_type")
            else ""
        )
        ...
        if new_shipment_type:
            ...
            if new_shipment_type == "LCL":
                ...
                logger.debug("Cleared container_type and container_count for LCL shipment")
            ...
            # If shipment_type is FCL, clear LCL-specific fields
                ...
                if "volume" not in new_data or not new_data.get("volume", "").strip():
                    if "volume" in merged:
                        del merged["volume"]
                logger.debug("Cleared weight/volume for FCL shipment (if not in new data)")

        # Merge remaining fields with recency priority. Empty strings count as "no update",
        # so existing values are preserved rather than cleared.
        for key, new_value in new_data.items():
            if key == "shipment_type":
                continue
            elif new_value and str(new_value).strip():
    ...
    def _merge_contact_info(
        ...
        # Empty strings count as "no update", so existing values are preserved.
        for key, new_value in new_data.items():
    ...
    def _merge_special_requirements(
        self, new_data: List[str], cumulative_data: List[str]
    ) -> List[str]:
    ...
    def merge_with_recency_priority(
        self, new_data: Dict[str, Any], cumulative_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        logger.info("Merging new data with cumulative data for thread")
        ...
        if missing_keys:
            logger.warning(f"Missing keys in merged data: {missing_keys}")
        else:
    ...
    def add_email_to_thread(self, thread_id: str, email_entry: EmailEntry) -> ThreadData:
        ...
        if email_entry.extracted_data:
            ...
            thread_data.cumulative_extraction = self.merge_with_recency_priority(
                email_entry.extracted_data, thread_data.cumulative_extraction
            )
    ...
    def update_cumulative_extraction(self, thread_id: str, new_extraction: Dict[str, Any]) -> bool:
        ...
        thread_data.cumulative_extraction = self.merge_with_recency_priority(
            new_extraction, thread_data.cumulative_extraction
        )
        thread_data.last_updated = datetime.now().isoformat()
    ...
    def update_context(
        self, thread_id: str, context_type: str, context_data: Dict[str, Any]
    ) -> bool:

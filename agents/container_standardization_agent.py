...
class ContainerStandardizationAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        # Default container type for fallback cases (most common type).
        self.default_container = "40DC"
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Handle alternative key
            ...
            return {
                "error": "No container_description, container_descriptions, or container_type provided"
            }
        ...
        if self.client:
            try:
                ...
                if (
                    ...
                    and llm_result.get("confidence", 0) >= self.confidence_threshold
                ):
                    return llm_result
                else:
                    self.logger.info("LLM result below threshold, using comprehensive fallback")
...
"""
            ...
            result["standardized_type"] = standard_type
            result["rate_fallback_type"] = self.rate_fallback_mapping.get(
    ...
    def _comprehensive_standardization(self, description: str) -> Dict[str, Any]:
        ...
        # Calculate scores
        score_data = self._calculate_container_scores(
            size_data, type_data, exact_data, special_data
        )

        # Calculate confidence
        confidence_data = self._calculate_confidence(
            score_data["scores"], score_data["matched_terms"], cleaned_text
        )

        # Generate suggestions
        suggestions = self._generate_suggestions(
            confidence_data["confidence"], score_data["best_match"], score_data["scores"]
        )
    ...
    def _detect_container_type(self, text: str) -> Dict[str, Any]:
        ...
        type_patterns = {
            "dry": [r"\b(?:DRY|DC|DV|GP|GENERAL)\b", r"\bSTANDARD\b", r"\bCONTAINER\b"],
            "reefer": [
                r"\b(?:REEFER|RF|RE|RH|REFRIGERAT|REFRIG|COOL|COLD|FROZEN)\b",
                r"\bTEMPERATURE\s*CONTROL\b",
            ],
            "high_cube": [r"\b(?:HIGH\s*CUBE|HC|HQ|CUBE)\b", r"\bHIGH\b.*\bCUBE\b"],
            "tank": [r"\b(?:TANK|TK|LIQUID|BULK)\b", r"\bTANK\s*CONTAINER\b"],
        }
    ...
    def _check_exact_phrases(self, text: str) -> Dict[str, Any]:
        ...
        return {"exact_match": exact_match, "has_exact_match": has_exact_match}

    def _handle_special_cases(self, text: str) -> Dict[str, Any]:
        """Handle special cases and edge conditions"""
        special_cases = {
            "20DC": [r"\bTWENTY\s*FOOT\s*DRY\b", r"\bTEU\s*DRY\b", r"\b20\s*FOOT\s*STANDARD\b"],
            "40DC": [r"\bFORTY\s*FOOT\s*DRY\b", r"\bFEU\s*DRY\b", r"\b40\s*FOOT\s*STANDARD\b"],
            "20RF": [
            ...
            "20TK": [r"\bTWENTY\s*FOOT\s*TANK\b", r"\bTEU\s*TANK\b", r"\b20\s*FOOT\s*LIQUID\b"],
        }
        ...
        return {"special_match": special_match, "is_special_case": is_special_case}

    def _calculate_container_scores(
        self, size_data: Dict, type_data: Dict, exact_data: Dict, special_data: Dict
    ) -> Dict[str, Any]:
        ...
        return {"scores": scores, "matched_terms": matched_terms, "best_match": best_match}

    def _calculate_confidence(
        self, scores: Dict[str, float], matched_terms: Dict[str, List], text: str
    ) -> Dict[str, Any]:
    ...
    def _generate_suggestions(
        self, confidence: float, best_match: str, scores: Dict[str, float]
    ) -> List[str]:
    ...
    def validate_container_type(self, container_type: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "rate_fallback": (
                self.rate_fallback_mapping.get(cleaned_type, cleaned_type) if is_valid else None
            ),
        }
...
def test_container_standardization_agent():
    ...
    print("=== Testing Container Standardization Agent ===")
    ...
    test_cases = [
        {"name": "Standard 20GP", "input": "20GP", "expected": "20DC"},
        {"name": "40 foot reefer", "input": "40 foot reefer", "expected": "40RF"},
        {"name": "High cube container", "input": "high cube", "expected": "40HC"},
        {"name": "Tank container", "input": "tank container", "expected": "20TK"},
        {"name": "TEU dry", "input": "TEU dry", "expected": "20DC"},
        {"name": "FEU standard", "input": "FEU standard", "expected": "40DC"},
        {"name": "20RF exact", "input": "20RF", "expected": "20RF"},
        {"name": "40HC exact", "input": "40HC", "expected": "40HC"},
        {"name": "Refrigerated 40ft", "input": "refrigerated 40ft", "expected": "40RF"},
        {
            "name": "Invalid input",
...
def test_llm_vs_fallback():
    ...
    for desc in test_descriptions:
        ...
        llm_type = (
            llm_result.get("standard_type", "Error")
            if llm_result.get("status") == "success"
            else "Error"
        )
        fallback_type = (
            fallback_result.get("standard_type", "Error")
        ...
        match = "PASS" if llm_type == fallback_type else "FAIL"

        print(
            f"{desc:11} | {llm_type:10} | {fallback_type:15} | {llm_conf:7.1f} | {fb_conf:6.1f} | {match}"
        )
...
def test_edge_cases():
    ...
    edge_cases = [
        ...
        {"name": "Special characters", "input": "@#$%^&*()"},
        {
            ...
            "input": "This is a very long container description that contains many words but no clear container type information",
        },
...
def test_validation_functionality():
    ...
    for test in validation_tests:
        ...
        status = "PASS" if is_valid == test["should_be_valid"] else "FAIL"
        print(
            f"{status} '{test['input']}' -> Valid: {is_valid} (Expected: {test['should_be_valid']})"
        )
...
def test_confidence_scoring():
    ...
    for test in confidence_tests:
        ...
        if result.get("status") == "success":
            ...
            status = "PASS" if is_high_confidence == test["expected_high"] else "FAIL"
            print(
                f"{status} '{test['input']}' -> Confidence: {confidence:.1f}% (Expected high: {test['expected_high']})"
            )
        else:
...
def test_detailed_analysis():
    ...
    if result.get("status") == "success":
        ...
        if all_scores:
            print("All Scores:")
            for container_type, score in sorted(
...
def quick_test():
    ...
    quick_tests = ["20GP", "40 foot reefer", "high cube", "tank container", "invalid input"]

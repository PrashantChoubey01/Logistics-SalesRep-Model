#!/usr/bin/env python3
"""Analyzes email threads and conversation context with a dedicated LLM.

Tracks conversation progression and thread state, detects vague-response patterns,
and is kept separate from classification to improve accuracy.
"""
...
class ThreadContextAnalyzerAgent(BaseAgent):
    """Analyzes thread context and conversation progression using a dedicated LLM."""

    def __init__(self):
        super().__init__("ThreadContextAnalyzerAgent")

        # Dedicated model for thread analysis
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        print("THREAD_ANALYZER: Starting LLM thread analysis...")
        ...
        print("THREAD_ANALYZER: LLM analysis complete")
    ...
    def _specialized_thread_analysis(
        self,
        email_text: str,
        subject: str,
        sender_email: str,
        ...
        thread_summary = self._prepare_detailed_thread_summary(
            thread_history, previous_classifications
        )
        conversation_pattern = self._analyze_conversation_pattern(
            thread_history, previous_classifications
        )
        vague_response_tracking = self._track_vague_responses(
            thread_history, previous_classifications
        )
        ...
        if "error" in llm_result:
            print(f"LLM thread analysis failed, using fallback: {llm_result['error']}")
            return self._fallback_thread_analysis(thread_history, previous_classifications)
    ...
    def _create_thread_analysis_prompt(
        ...
        previous_classifications: List[Dict],
        customer_context: Dict,
        forwarder_context: Dict,
    ) -> str:
...
"""
    ...
    def _prepare_detailed_thread_summary(
        self, thread_history: List, previous_classifications: List
    ) -> str:
        """
        ...
        for i, (email, classification) in enumerate(
            zip(thread_history, previous_classifications), 1
        ):
            ...
            if is_vague:
                summary_parts.append("     ⚠️ Vague response detected")
    ...
    def _analyze_conversation_pattern(
        ...
        email_types = [
            classification.get("email_type", "unknown")
    ...
    def _track_vague_responses(self, thread_history: List, previous_classifications: List) -> str:
        ...
        tracking_parts = ["Vague Response Tracking:"]
        ...
        if len(vague_responses) >= 2:
            tracking_parts.append(
                "  ⚠️ Multiple vague responses detected - escalation may be needed"
            )

        return "\n".join(tracking_parts)

    def _build_thread_context(
        self, thread_history: List[Dict], previous_classifications: List[Dict]
    ) -> str:
    ...
    def _enhance_thread_analysis_result(
        self,
        llm_result: Dict,
        thread_history: List,
        previous_classifications: List,
        ...
        enhanced_result = {
            **llm_result,
            ...
            "conversation_analysis": {
                "email_type_distribution": self._get_email_type_distribution(
                    previous_classifications
                ),
                "vague_response_positions": self._get_vague_response_positions(
                    previous_classifications
                ),
            ...
            "analysis_details": {
                ...
                "confidence_factors": self._extract_thread_confidence_factors(
                    llm_result, thread_history
                ),
            },
        }
    ...
    def _analyze_conversation_progression(
        self, thread_history: List, previous_classifications: List
    ) -> str:
        """
    ...
    def _extract_thread_confidence_factors(
        self, llm_result: Dict, thread_history: List
    ) -> List[str]:
        """
    ...
    def _fallback_thread_analysis(
        self, thread_history: List, previous_classifications: List
    ) -> Dict[str, Any]:
        """
        Fallback thread analysis when specialized LLM is not available.
        """
        print("Using fallback thread analysis (LLM not available)")
        ...
        return {
            ...
            "escalation_reason": (
                "Fallback analysis - multiple vague responses"
                if vague_count >= 3
                else "Fallback analysis - long thread"
            ),
    ...
    def _generate_thread_analysis_response(
        self, prompt: str, function_schema: Dict
    ) -> Dict[str, Any]:
        """
...
def test_thread_analyzer():
    """Test the Thread Context Analyzer Agent."""
    print("Testing Thread Context Analyzer Agent")
    print("=" * 60)

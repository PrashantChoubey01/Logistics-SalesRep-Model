...
Generates acknowledgment responses for sales person, forwarder, and customer
emails using the sales team configuration with Searates By DP World branding.
"""
...
class AcknowledgmentResponseAgent(BaseAgent):
    ...
    def _generate_sales_person_acknowledgment(
        self,
        sender_email: str,
        sender_details: Dict[str, Any],
        email_content: str,
        assigned_sales_person: Dict[str, Any],
    ) -> Dict[str, Any]:
...
📞 {assigned_sales_person.get('phone', '+1-555-0123')}"""
    ...
    def _generate_customer_acknowledgment(
        self, sender_email: str, email_content: str, assigned_sales_person: Dict[str, Any]
    ) -> Dict[str, Any]:
...
🌐 www.searates.com"""
    ...
    def _extract_rate_info_from_email(self, email_content: str) -> Dict[str, Any]:
        ...
        transit_match = re.search(
            r"transit\s*time[:\s]*(\d+)\s*days?", email_content, re.IGNORECASE
        )
        if transit_match:
        ...
        valid_match = re.search(
            r"valid\s*until[:\s]*([A-Za-z]+\s+\d+,\s+\d{4})", email_content, re.IGNORECASE
        )
        if valid_match:
    ...
    def _generate_fallback_acknowledgment(
        self, sender_email: str, sender_type: str
    ) -> Dict[str, Any]:

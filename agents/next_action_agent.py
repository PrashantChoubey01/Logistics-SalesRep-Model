...
class NextActionAgent(BaseAgent):
    ...
    def _determine_next_action(
        ...
        try:
            ...
            missing_fields = validation_results.get("overall_validation", {}).get(
                "missing_fields", []
            )

            # Treat country-only origin/destination as missing (a specific port is required).
            # This must run before the FCL/LCL rules below.
            ...
            # Flag as missing only when origin is empty but a country was given (country-only input).
            # A populated origin with a country is a valid port and is left alone.
            if not origin and origin_country:
                if "Origin (specific port required)" not in missing_fields:
                    ...
                    self.logger.warning(
                        f"Origin is a country ({origin_country}), not a specific port. Adding to missing fields."
                    )
            elif not origin:
            ...
            if not destination and destination_country:
                if "Destination (specific port required)" not in missing_fields:
                    ...
                    self.logger.warning(
                        f"Destination is a country ({destination_country}), not a specific port. Adding to missing fields."
                    )
            elif not destination:
                if "Destination" not in missing_fields:
                    missing_fields.append("Destination")

            # Fallback: use port_lookup_result when the country fields were not extracted
            ...
            if port_lookup_result and not origin_country and not destination_country:
                ...
                if origin_result and origin_result.get("is_country", False):
                    if "Origin (specific port required)" not in missing_fields:
                        missing_fields.append("Origin (specific port required)")
                        self.logger.warning(
                            f"Origin is a country ({origin_result.get('country', 'unknown')}), not a specific port. Adding to missing fields."
                        )
                ...
                if destination_result and destination_result.get("is_country", False):
                    if "Destination (specific port required)" not in missing_fields:
                        missing_fields.append("Destination (specific port required)")
                        self.logger.warning(
                            f"Destination is a country ({destination_result.get('country', 'unknown')}), not a specific port. Adding to missing fields."
                        )

            missing_fields = self._apply_fcl_lcl_rules_to_missing_fields(
                missing_fields, extracted_data
            )

            customer_confirmed = self._check_customer_confirmation(
                conversation_state, email_classification
            )

            # Customer confirmation cannot override missing mandatory fields; clarification comes first.
            if customer_confirmed and missing_fields and len(missing_fields) > 0:
                self.logger.warning(
                    f"Customer confirmed but mandatory fields missing: {missing_fields}. Requiring clarification first."
                )
...
"""
            ...
            # Block confirmation while any mandatory field is missing, even if the customer confirmed.
            ...
            if missing_fields and len(missing_fields) > 0:
                if next_action in [
                    ...
                    self.logger.warning(
                        f"LLM attempted to send confirmation with missing fields: {missing_fields}. Overriding to clarification (customer_confirmed={customer_confirmed})."
                    )
                    result["next_action"] = "send_clarification_request"
                    result["response_type"] = "clarification"
                    result["reasoning"] = (
                        f"OVERRIDE: Mandatory fields missing ({', '.join(missing_fields)}). Confirmation requires ALL mandatory fields, including specific ports (not countries). Customer confirmation cannot override missing mandatory information. "
            ...
            if result.get("next_action") not in self.available_actions:
                self.logger.warning(
                    f"Invalid next_action: {result.get('next_action')}, defaulting to escalate_to_human"
                )
            ...
            self.logger.info(
                f"Next action determination successful: {result['next_action']} (priority: {result['action_priority']})"
            )
    ...
    def _format_data_summary(
        self, extracted_data: Dict[str, Any], missing_fields: List[str]
    ) -> str:
    ...
    def _apply_fcl_lcl_rules_to_missing_fields(
        self, missing_fields: List[str], extracted_data: Dict[str, Any]
    ) -> List[str]:
        """Apply FCL/LCL business rules to filter missing fields."""
        shipment_details = extracted_data.get("shipment_details", {})

        # Prefer the shipment_type extracted directly from the email.
        ...
        if isinstance(container_type, dict):
            container_type = container_type.get("standardized_type", "") or container_type.get(
                "standard_type", ""
            )
        container_type = str(container_type).strip() if container_type else ""

        # Priority: extracted shipment_type > special_requirements > container_type
        is_fcl = None

        if shipment_type == "LCL":
            is_fcl = False
            if (
                "container_count" in missing_fields
                or "Number of containers" in " ".join(missing_fields)
                or "Quantity (number of containers)" in " ".join(missing_fields)
            ):
                missing_fields = [
                    f
                ...
                print(
                    f"NEXT_ACTION: Removed container_count from missing fields (LCL shipment - extracted shipment_type: {shipment_type})"
                )
            ...
            if is_explicitly_lcl:
                ...
                if (
                    ...
                    print(
                        f"NEXT_ACTION: Removed container_count from missing fields (LCL shipment with special_requirements: {special_requirements})"
                    )
            elif is_explicitly_fcl:
                is_fcl = True
            elif container_type and container_type.upper() in [
                "20GP",
                "40GP",
                "40HC",
                "20RF",
                ...
                # Container type present but no shipment_type - assume FCL
                is_fcl = True
            else:
                is_fcl = False

        if is_fcl:
            # FCL: volume is not required (container type determines volume)
            if "volume" in missing_fields:
                missing_fields.remove("volume")
                print(
                    f"NEXT_ACTION: Removed volume from missing fields (FCL shipment with container type: {container_type})"
                )
        else:
            # LCL: container_count is not required
            if (
                "container_count" in missing_fields
                or "Number of containers" in " ".join(missing_fields)
                or "Quantity (number of containers)" in " ".join(missing_fields)
                ...
                print("NEXT_ACTION: Removed container_count from missing fields (LCL shipment)")

        return missing_fields

    def _check_customer_confirmation(
        self, conversation_state: str, email_classification: Dict[str, Any]
    ) -> bool:
...
def test_next_action_agent():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "conversation_state": "new_request",
            "extracted_data": {"origin": "Shanghai", "destination": "Long Beach"},
        ...
        },
        {
            ...
            "extracted_data": {"rate_amount": 2500, "currency": "USD", "valid_until": "2024-01-15"},

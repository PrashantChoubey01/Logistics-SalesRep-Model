#!/usr/bin/env python3
"""Classifies emails and senders with a dedicated LLM.

Provides confidence scoring and vague-content detection, and is kept separate from
other agents to improve classification accuracy.
"""
...
class UnifiedEmailClassifierAgent(BaseAgent):
    """Classifies email type and sender using a dedicated LLM."""
    ...
    def __init__(self):
        ...
        try:
            ...
            print("SalesTeamManager not available")
        ...
        try:
            ...
            print("ForwarderManager not available")
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        print("UNIFIED_CLASSIFIER: Starting LLM classification...")

        # Focus on email content, not subject
    ...
    def _classify_sender_type(self, sender_email: str) -> Dict[str, Any]:
        ...
        if self.sales_team_manager:
            try:
                ...
                print(f"Error checking sales team: {e}")

        if self.forwarder_manager:
            try:
                forwarder = self.forwarder_manager.get_forwarder_by_email(sender_email)
                if forwarder:
                    return {"type": "forwarder", "details": forwarder, "confidence": 1.0}
            except Exception as e:
                print(f"Error checking forwarders: {e}")

        # Not in sales team or forwarders: treat as customer
        return {"type": "customer", "details": {"email": sender_email}, "confidence": 0.8}

    def _specialized_classification(
        self,
        email_text: str,
        sender_email: str,
        thread_id: str,
        ...
        sender_classification: Dict[str, Any],
    ) -> Dict[str, Any]:
        """
        Use specialized classification LLM for email classification.
        """
        if not self.client:
            return self._escalate_classification_failure(
                email_text, sender_email, "LLM client not available"
            )

        # Prepare context for classification LLM
        thread_summary = self._prepare_thread_summary(thread_history)
        sender_context = self._prepare_sender_context(
            customer_context, forwarder_context, sender_email
        )
                ...
                "properties": {
                    "email_type": {
                        ...
                        "enum": [
                            ...
                            "forwarder_rate_quote",
                            "forwarder_acknowledgment",
                            "forwarder_confirmation",
                            "sales_person_inquiry",
                            "sales_person_update",
                            "sales_person_acknowledgment",
                            "escalation_request",
                            "general_inquiry",
                            "complaint",
                            "unknown",
                    ...
                    "intent": {"type": "string", "description": "Primary intent of the email"},
                    "reasoning": {
                        "type": "string",
        ...
        if "error" in llm_result:
            print(f"LLM classification failed: {llm_result['error']}")
            return self._escalate_classification_failure(
                email_text, sender_email, llm_result["error"]
            )
...
"""
    ...
    def _prepare_thread_summary(self, thread_history: List) -> str:
        ...
        vague_count = sum(
            1 for email in thread_history if email.get("classification", {}).get("is_vague", False)
        )
        if vague_count > 0:
    ...
    def _prepare_sender_context(
        self, customer_context: Dict, forwarder_context: Dict, sender_email: str
    ) -> str:
        """
        ...
        # Check if sender is known forwarder
            ...
            if forwarder_info:
                ...
                context_parts.append(
                    f"  Specialties: {forwarder_info.get('specialties', 'Unknown')}"
                )
    ...
    def _enhance_classification_result(
        self, llm_result: Dict, email_text: str, sender_email: str, thread_history: List
    ) -> Dict[str, Any]:
        """
    ...
    def _escalate_classification_failure(
        self, email_text: str, sender_email: str, error: str
    ) -> Dict[str, Any]:
        """
...
def test_specialized_classifier():
    """Test the Unified Email Classifier Agent."""
    print("Testing Unified Email Classifier Agent")
    print("=" * 60)
    ...
    test_cases = [
        ...
        },
        {
            ...
            "thread_history": [{"classification": {"email_type": "customer_quote_request"}}],
        },
        {
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if result.get("is_vague", False):
            print(f"Vague Content: {result.get('vague_reason', 'Unknown reason')}")

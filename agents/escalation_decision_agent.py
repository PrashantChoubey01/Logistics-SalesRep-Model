...
Uses a dedicated LLM to decide when and how to escalate issues, weighing
multiple factors and context. Classifies the escalation type and assesses
...
class EscalationDecisionAgent(BaseAgent):
    """Decides when and how to escalate issues using a dedicated LLM.

    Performs multi-factor, context-aware analysis with priority/urgency
    assessment and escalation routing.
    """

    def __init__(self):
        super().__init__("EscalationDecisionAgent")

        # Dedicated model focused on decision making.
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print("ESCALATION_DECISION: Specialized LLM analysis complete")
    ...
    def _specialized_escalation_decision(
        self,
        email_text: str,
        subject: str,
        sender_email: str,
        ...
        if "error" in llm_result:
            print(
                f"Specialized LLM escalation decision failed, using fallback: {llm_result['error']}"
            )
            return self._fallback_escalation_decision(classification_result, thread_context)
    ...
    def _create_escalation_decision_prompt(
        self,
        email_text: str,
        subject: str,
        sender_email: str,
...
"""
    ...
    def _prepare_escalation_context(self, escalation_history: List) -> str:
        ...
        recent_escalations = [
            e for e in escalation_history if e.get("priority") in ["high", "urgent"]
        ]
    ...
    def _enhance_escalation_result(
        self,
        llm_result: Dict,
        email_text: str,
        subject: str,
        ...
        enhanced_result = {
            **llm_result,
            ...
            "escalation_context": {
                ...
                "recent_high_priority": len(
                    [e for e in escalation_history[-5:] if e.get("priority") in ["high", "urgent"]]
                ),
            },
            "analysis_details": {
    ...
    def _fallback_escalation_decision(
        self, classification_result: Dict, thread_context: Dict
    ) -> Dict[str, Any]:
        """
        Fallback escalation decision when specialized LLM is not available.
        """
        print("Using fallback escalation decision (Specialized LLM not available)")

        # Simple fallback logic
        escalation_needed = (
            classification_result.get("escalation_needed", False)
            or thread_context.get("vague_pattern_detected", False)
            or thread_context.get("vague_response_count", 0) >= 3
        )

        if escalation_needed:
            escalation_type = (
                "vague_communication"
                if thread_context.get("vague_pattern_detected", False)
    ...
    def _generate_escalation_decision_response(
...
def test_escalation_decision():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "customer_context": {
                "priority": "standard",
                "customer_info": {"name": "Jane Doe", "company": "Small Business Ltd."},
            },
            "forwarder_context": {},
            "escalation_history": [],
        },
        {
            ...
            "customer_context": {
                "priority": "vip",
                "customer_info": {"name": "VIP Customer", "company": "Large Corporation"},
            },
            "forwarder_context": {},
            "escalation_history": [],
        },

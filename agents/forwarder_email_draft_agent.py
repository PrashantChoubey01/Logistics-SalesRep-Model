...
"""
    ...
    def _generate_email_drafts(
        self,
        assigned_forwarders: List[Dict[str, Any]],
        ...
        try:
            ...
            function_schema = {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "total_emails_generated": {"type": "integer"},
                        "email_strategy": {"type": "string"},
                        "confidentiality_level": {"type": "string"},
                    },
                },
            }

            # Resolve origin/destination, preferring port codes when available
            ...
            if port_lookup_result:
                if port_lookup_result.get("origin") and port_lookup_result["origin"].get(
                    "port_code"
                ):
                ...
                if port_lookup_result.get("destination") and port_lookup_result["destination"].get(
                    "port_code"
                ):
                    dest_code = port_lookup_result["destination"]["port_code"]
                    dest_name = port_lookup_result["destination"].get(
                        "port_name", destination_display
                    )
            ...
            shipment_info = {
                ...
                "container_count": shipment_details.get(
                    "container_count", shipment_details.get("quantity", "")
                ),
                "weight": shipment_details.get("weight", ""),
                "volume": shipment_details.get("volume", ""),
                "shipment_date": shipment_details.get(
                    "shipment_date", shipment_details.get("requested_dates", "")
                ),
...
"""
            ...
            self.logger.info(
                f"Email draft generation successful: {result['total_emails_generated']} emails generated (confidentiality: {result['confidentiality_level']})"
            )
...
def test_forwarder_email_draft_agent():
    ...
    test_input = {
        ...
        "sales_manager_id": "SM001",
        "customer_email_content": "Dear Logistics Solutions Inc., I am interested in a rate for a shipment from Dubai to Mumbai. The shipment consists of 2 units of electronics weighing 25,000 kg and occupying 35 CBM. Could you please provide a competitive rate for this route? I would appreciate a response within 24 hours. Thank you!",
    }

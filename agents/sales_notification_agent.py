...
class SalesNotificationAgent(BaseAgent):
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        forwarder_rate_request_email = input_data.get(
            "forwarder_rate_request_email", ""
        )  # Rate request sent to forwarder
        ...
        return self._generate_sales_notification(
            notification_type,
            customer_details,
            shipment_details,
            forwarder_rates,
    ...
    def _generate_sales_notification(
        ...
        forwarder_rates: List[Dict[str, Any]],
        forwarder_details: Dict[str, Any],
        forwarder_email_content: str,
        forwarder_rate_request_email: str,
        timeline_information: Dict[str, Any],
...
"""
            ...
            if len(str(result.get("body") or "").strip()) < 60:
                self.logger.warning(
                    "Sales notification body missing/short — using deterministic collated fallback"
                )
            ...
            if not str(result.get("subject") or "").strip():
                result["subject"] = (
                    f"[Collated Rate] Forwarder quote ready for markup — Thread {thread_id}"
                )
            ...
            if thread_id and result.get("body"):
                ...
                if (
                    f"Thread ID: {thread_id}" not in result.get("body", "")
                    and f"thread_id: {thread_id}" not in result.get("body", "").lower()
                ):
            ...
            if (
                forwarder_email_content
                and forwarder_email_content.strip()
                and "No forwarder email received yet" not in forwarder_email_content
            ):
                # Check if it's already in the body
                if (
                    result.get("body")
                    and "FORWARDER EMAIL RECEIVED" not in result.get("body", "").upper()
                ):
                    result["body"] = result["body"] + "\n\n" + forwarder_email_content.strip()
                elif (
                    result.get("body")
                    and "FORWARDER EMAIL RECEIVED" in result.get("body", "").upper()
                ):
            ...
            self.logger.info(
                f"Sales notification generated successfully: {notification_type} (priority: {result['priority']})"
            )
    ...
    def _build_collated_body(
        self,
        customer_summary: str,
        shipment_summary: str,
        forwarder_summary: str,
        rates_summary: str,
    ...
    def _format_rates_summary(self, forwarder_rates: List[Dict[str, Any]]) -> str:
        ...
        for i, rate in enumerate(forwarder_rates, 1):
            ...
            if rate.get("rate"):
                rate_details.append(
                    f"Rate: ${rate.get('rate'):,.2f} USD"
                    if isinstance(rate.get("rate"), (int, float))
                    else f"Rate: {rate.get('rate')} USD"
                )
            elif rate.get("rates_with_dthc"):
                rate_value = rate.get("rates_with_dthc")
                rate_details.append(
                    f"Total Rate: ${rate_value:,.2f} USD"
                    if isinstance(rate_value, (int, float))
                    else f"Total Rate: {rate_value} USD"
                )
            elif rate.get("rates_with_othc"):
                rate_value = rate.get("rates_with_othc")
                rate_details.append(
                    f"Rate with OTHC: ${rate_value:,.2f} USD"
                    if isinstance(rate_value, (int, float))
                    else f"Rate with OTHC: {rate_value} USD"
                )
            elif rate.get("rates_without_thc"):
                rate_value = rate.get("rates_without_thc")
                rate_details.append(
                    f"Freight Rate: ${rate_value:,.2f} USD"
                    if isinstance(rate_value, (int, float))
                    else f"Freight Rate: {rate_value} USD"
                )

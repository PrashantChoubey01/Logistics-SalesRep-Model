...
Generates confirmation responses asking customers to confirm extracted
information before proceeding with the quote.
"""
...
class ConfirmationResponseAgent(BaseAgent):
    ...
    def generate_confirmation_response(
        ...
        try:
            ...
            response_body = self._build_response_body(
                template, customer_name, extracted_data, confirmation_info, agent_info, rate_info
            )
            ...
            logger.info(
                f"Generated confirmation response with {len(extracted_data)} data categories"
            )
            return response
    ...
    def _format_confirmation_info(
            ...
            if any(shipment.values()):
                ...
                if shipment.get("origin"):
                    ...
                    if port_lookup_result and port_lookup_result.get("origin"):
                        ...
                        logger.debug(
                            f"Origin formatting: port_name={port_name}, port_code={port_code}, confidence={confidence:.2f}, country={country}, use_port_name={use_port_name}"
                        )
                        ...
                        logger.debug(
                            f"No port_lookup_result for origin, using raw value: {origin_display}"
                        )
                ...
                if shipment.get("destination"):
                    ...
                    if port_lookup_result and port_lookup_result.get("destination"):
                        ...
                        original_port_name = destination_result.get(
                            "original_port_name", shipment["destination"]
                        )
                        port_name = (
                            destination_result.get("port_name")
                            or original_port_name
                            or shipment["destination"]
                        )
                        ...
                        logger.debug(
                            f"Destination formatting: port_name={port_name}, port_code={port_code}, confidence={confidence:.2f}, country={country}, use_port_name={use_port_name}, original_port_name={original_port_name}"
                        )
                        ...
                        logger.debug(
                            f"No port_lookup_result for destination, using raw value: {destination_display}"
                        )
        ...
        return (
            ...
            else "No specific details to confirm."
        )
    ...
    def _clean_additional_notes(self, notes: str) -> str:
        ...
        for line in lines:
            ...
            line = re.sub(
                r"Please provide rates and transit time[;,]?\s*", "", line, flags=re.IGNORECASE
            )
        ...
        if cleaned_lower in job_titles or cleaned_lower in [
            "best regards",
            "regards",
            "sincerely",
            "thank you",
    ...
    def _build_response_body(
        ...
        if rate_info:
            ...
            response_parts.extend(["**Rate Information:**", "", rate_text, ""])
    ...
    def _generate_fallback_response(
        self, extracted_data: Dict[str, Any], customer_name: str
    ) -> Dict[str, Any]:
...
sales@searates.com""",
    ...
    def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if container_standardization_result and isinstance(
                container_standardization_result, dict
            ):

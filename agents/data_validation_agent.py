...
Uses a dedicated LLM to validate extracted information for quality, completeness,
and consistency. Covers format, business-rule, and cross-field validation, data
quality assessment, and error-correction suggestions.
"""
...
class DataValidationAgent(BaseAgent):
    """Validates extracted data with a dedicated LLM.

    Performs cross-field and business-rule validation, consistency checks, data
    quality scoring, and error-correction recommendations.
    """

    def __init__(self):
        super().__init__("DataValidationAgent")

        # Dedicated model focused on validation logic.
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        print("DATA_VALIDATOR: Starting specialized LLM data validation...")
        ...
        print("DATA_VALIDATOR: Specialized LLM validation complete")
    ...
    def _specialized_data_validation(
        self,
        ...
        """
        if not self.client:
            return self._fallback_data_validation(
        ...
        if "error" in llm_result:
            print(f"Specialized LLM data validation failed, using fallback: {llm_result['error']}")
            return self._fallback_data_validation(
                shipment_details, contact_info, rate_info, timeline_info
            )
...
"""
    ...
    def _enhance_validation_result(
        ...
        try:
            ...
            validation_details = self._prepare_validation_details(
                llm_result, shipment_details, contact_info, rate_info, timeline_info
            )
    ...
    def _prepare_validation_details(
        self,
        llm_result: Dict,
        shipment_details: Dict,
        contact_info: Dict,
        ...
        try:
            ...
            if shipment_details:
                ...
                if is_fcl:
                    if not container_type:
                        validation_details["shipment_details"]["issues"].append(
            ...
            if contact_info:
                ...
                if not contact_info.get("name") and not contact_info.get("email"):
                    validation_details["contact_information"]["issues"].append(
            ...
            if rate_info:
                ...
                if not rate_info.get("total_rate"):
                    validation_details["rate_information"]["issues"].append(
            ...
            if timeline_info:
                ...
                if not timeline_info.get("requested_dates"):
                    validation_details["timeline_information"]["issues"].append(
                        "Missing requested_dates"
                    )
            ...
            validation_details = {"error": f"Validation details preparation failed: {str(e)}"}
    ...
    def _prepare_quality_breakdown(
        self, llm_result: Dict, quality_factors: List[str]
    ) -> Dict[str, float]:
        """
    ...
    def _fallback_data_validation(
        self, shipment_details: Dict, contact_info: Dict, rate_info: Dict, timeline_info: Dict
    ) -> Dict[str, Any]:
        """
        Fallback data validation when specialized LLM is not available.
        """
        print("Using fallback data validation (Specialized LLM not available)")
        ...
        total_fields = (
            len(shipment_details) + len(contact_info) + len(rate_info) + len(timeline_info)
        )
    ...
    test_cases = [
        {
            ...
            "context_data": {"customer_priority": "high", "service_type": "ocean_freight"},
            "previous_validation": {},
        },
        {
            ...
            "context_data": {"customer_priority": "standard", "service_type": "ocean_freight"},
            "previous_validation": {
                "overall_quality": 0.3,

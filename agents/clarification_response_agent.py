...
Generates clarification responses using the LLM while sharing extracted data
...
class ClarificationResponseAgent(BaseAgent):
    ...
    def load_context(self):
        """No predefined templates; responses are generated dynamically by the LLM."""
        pass
    ...
    def generate_clarification_response(
        ...
        port_lookup_result: Dict[str, Any] = None,
    ) -> Dict[str, Any]:
        """
        ...
        """
        try:
            logger.debug("Generating clarification response")
            logger.debug(f"Extracted data: {json.dumps(extracted_data, indent=2)}")
            logger.debug(
                f"Port lookup result: {json.dumps(port_lookup_result, indent=2) if port_lookup_result else 'None'}"
            )
            ...
            logger.info(
                f"Generating clarification response for {len(missing_fields)} missing fields"
            )
            ...
            return self._generate_fallback_response(
                missing_fields, customer_name, extracted_data, port_lookup_result
            )

    def _format_extracted_info(
        ...
        if "shipment_details" in extracted_data:
            ...
            origin_country = (
                shipment.get("origin_country", "").strip() if shipment.get("origin_country") else ""
            )
            if origin:
                has_any_data = True
            ...
            destination = (
                shipment.get("destination", "").strip() if shipment.get("destination") else ""
            )
            destination_country = (
                shipment.get("destination_country", "").strip()
                if shipment.get("destination_country")
                else ""
            )
            if destination:
                has_any_data = True
                ...
                if port_lookup_result and port_lookup_result.get("destination"):
                    ...
                    if destination_result.get("is_country", False):
                        ...
                        original_port_name = destination_result.get(
                            "original_port_name", destination
                        )
                        port_name = (
                            destination_result.get("port_name") or original_port_name or destination
                        )
            ...
            shipment_type = (
                shipment.get("shipment_type", "").strip().upper()
                if shipment.get("shipment_type")
                else ""
            )
            is_lcl = shipment_type == "LCL"
            is_fcl = shipment_type == "FCL" or (
                not shipment_type and shipment.get("container_type")
            )

            # FCL-specific fields (only show for FCL shipments)
            if is_fcl and not is_lcl:
                container_type = (
                    shipment.get("container_type", "").strip()
                ...
                container_count = (
                    shipment.get("container_count", "").strip()
            ...
            requested_dates = (
                timeline_info.get("requested_dates", "").strip()
        ...
        return (
            ...
            else "No specific details provided yet."
        )
    ...
    def _generate_llm_response(
        ...
        tone: str,
        port_lookup_result: Dict[str, Any],
    ) -> str:
...
"""
        ...
        try:
            ...
            if (
                extracted_info
                and extracted_info.strip()
                and extracted_info != "No specific details provided yet."
            ):
            ...
            if (
                missing_info
                and missing_info.strip()
                and missing_info != "All required information has been provided."
            ):
                full_response_parts.append("")
    ...
    def _generate_fallback_response(
        ...
        subject = (
            self._generate_subject_line(extracted_data, port_lookup_result)
            if extracted_data

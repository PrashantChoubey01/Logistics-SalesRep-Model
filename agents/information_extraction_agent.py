#!/usr/bin/env python3
"""Extracts structured information from emails with recency-based priority and cumulative coalescing."""
...
class InformationExtractionAgent(BaseAgent):
    """Extracts information from emails with recency-based priority."""
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            print("INFORMATION_EXTRACTOR: Starting LLM information extraction...")
            ...
            print("Information Extraction Process:")
            ...
            if not new_email_extraction:
                print("   New email extraction failed")
                return self._create_error_result("New email extraction failed")

            print("   New email extraction completed")
            ...
            if cumulative_extraction:
                ...
                print("   No cumulative extraction found (new thread)")
            ...
            final_extraction = self.thread_manager.merge_with_recency_priority(
                new_data=new_email_extraction, cumulative_data=cumulative_extraction
            )
            ...
            update_success = self.thread_manager.update_cumulative_extraction(
                thread_id=thread_id, new_extraction=new_email_extraction
            )

            if update_success:
                print("   Cumulative extraction updated")
            else:
                print("   Failed to update cumulative extraction")
    ...
    def _extract_from_new_email_only(
        self, email_text: str, subject: str, sender: str
    ) -> Optional[Dict[str, Any]]:
        """Extract information from new email only"""
        try:
            if not self.client:
                print("   LLM not loaded, attempting to load...")
                self.load_context()
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "origin": {
                                "type": "string",
                                "description": "Origin port/city name ONLY (NEVER store country names here). If input is 'Los Angeles, USA', extract 'Los Angeles'. If input is just 'USA' (country only), leave this EMPTY. Only extract if it's a specific port or city name.",
                            },
                            "origin_country": {
                                "type": "string",
                                "description": "Country name for origin. If input is 'Los Angeles, USA' or 'Shanghai, China', extract the country part ('United States' or 'China'). If input is just a country like 'USA', extract the full country name ('United States'). Leave empty if no country is mentioned.",
                            },
                            "destination": {
                                "type": "string",
                                "description": "Destination port/city name ONLY (NEVER store country names here). If input is 'Mumbai, India', extract 'Mumbai'. If input is just 'India' (country only), leave this EMPTY. Only extract if it's a specific port or city name.",
                            },
                            "destination_country": {
                                "type": "string",
                                "description": "Country name for destination. If input is 'Mumbai, India' or 'Rotterdam, Netherlands', extract the country part ('India' or 'Netherlands'). If input is just a country like 'India', extract the full country name. Leave empty if no country is mentioned.",
                            },
                            "container_type": {
                                "type": "string",
                                "description": "Container type (20GP, 40GP, 40HC, etc.)",
                            },
                            "container_count": {
                                "type": "string",
                            ...
                            "commodity": {
                                "type": "string",
                                "description": "Type of goods/commodity",
                            },
                            ...
                            "requested_dates": {
                                "type": "string",
                                "description": "Requested shipping dates",
                            },
                            "transit_time": {
                                "type": "string",
                                "description": "Transit time requirements",
                            },
                            ...
                            "shipment_type": {
                                "type": "string",
                                "description": "Shipment type: 'FCL' (Full Container Load) or 'LCL' (Less than Container Load). Extract directly if mentioned (e.g., 'LCL shipment', 'FCL shipment', 'full container', 'less than container'). If not explicitly mentioned, leave empty.",
                            },
            ...
            if response.choices and response.choices[0].message.tool_calls:
                ...
                try:
                    ...
                    # Try to recover valid JSON from a noisy response
                    try:
                ...
                # If shipment_type is absent but a container_type exists, default to FCL.
                extracted_shipment_type = (
                    extracted_data.get("shipment_type", "").strip().upper()
                    if extracted_data.get("shipment_type")
                    else ""
                )
                ...
                nested_data = {
                    "shipment_details": {
                        ...
                        "shipment_type": extracted_shipment_type,
                    },
                    "contact_information": {

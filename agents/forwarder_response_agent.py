#!/usr/bin/env python3
"""Handles forwarder communications: extracts rate information and generates acknowledgments."""
...
class ForwarderResponseAgent(BaseAgent):
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not forwarder_name or forwarder_name == "Forwarder":
                forwarder_name = self._extract_forwarder_name_from_email(
                    email_data, forwarder_email
                )
            ...
            response = self._generate_forwarder_response(
                email_data, forwarder_details, conversation_state, rate_info
            )

            # Always generate acknowledgment for forwarder emails
            acknowledgment = self._generate_forwarder_acknowledgment(
                forwarder_name, email_data, rate_info
            )

            # Generate collate email for sales team if rate information is present
            collate_email = None
            if (
                rate_info.get("rates_with_othc")
                or rate_info.get("rates_with_dthc")
                or rate_info.get("rates_without_thc")
            ):
    ...
    def _extract_forwarder_name_from_email(
        self, email_data: Dict[str, Any], forwarder_email: str = ""
    ) -> str:
    ...
    def _extract_rate_information(self, email_text: str) -> Dict[str, Any]:
        ...
        # "Rate:" pattern (most common in forwarder emails)
        rate_patterns = [
    ...
    def _generate_forwarder_response(
        self,
    ...
    def _generate_rate_quote_acknowledgment(
        self, forwarder_name: str, email_data: Dict[str, Any], rate_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if rate_info["origin_port"] and rate_info["destination_port"]:
            rate_details.append(
                f"Route: {rate_info['origin_port']} to {rate_info['destination_port']}"
            )
            ...
            if not response_body or len(response_body.strip()) < 50:
                ...
                if rate_info.get("rates_without_thc"):
                    rate_breakdown.append(
                        f"  + Ocean Freight: USD {rate_info['rates_without_thc']:,.2f}"
                    )
                if rate_info.get("rates_with_othc"):
                    rate_breakdown.append(
                        f"  + With Origin THC: USD {rate_info['rates_with_othc']:,.2f}"
                    )
                if rate_info.get("rates_with_dthc"):
                    rate_breakdown.append(
                        f"  + Total Rate: USD {rate_info['rates_with_dthc']:,.2f}"
                    )
                ...
                rate_details_text = (
                    "\n".join(rate_details_list)
                    if rate_details_list
                    else "Rate details are being reviewed."
                )
...
sales@searates.com"""
    ...
    def _generate_information_request_response(
        self, forwarder_name: str, email_data: Dict[str, Any]
    ) -> Dict[str, Any]:
...
sales@searates.com""",
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional sales representative at SeaRates by DP World. Generate friendly, professional, and human-like responses.",
                    },
    ...
    def _generate_collate_email_for_sales(
        self,
        email_data: Dict[str, Any],
        forwarder_details: Dict[str, Any],
        rate_info: Dict[str, Any],
...
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    def _generate_forwarder_acknowledgment(
        self, forwarder_name: str, email_data: Dict[str, Any], rate_info: Dict[str, Any]
    ) -> Dict[str, Any]:
...
sales@searates.com"""
        ...
        return {"subject": subject, "body": body, "type": "forwarder_acknowledgment"}

    def generate_forwarder_assignment_acknowledgment(
        self, forwarder_assignment: Dict[str, Any], customer_data: Dict[str, Any]
    ) -> Dict[str, Any]:
...
def test_forwarder_response_agent():
    ...
    print("Testing Forwarder Response Agent")
    print("=" * 50)
...
dhl.global.forwarding@logistics.com""",
    ...
    input_data = {
        ...
        "conversation_state": {"conversation_state": "thread_forwarder_interaction"},
    }
    ...
    print("\nForwarder Response Test Completed")

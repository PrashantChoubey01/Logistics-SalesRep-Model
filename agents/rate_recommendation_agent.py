...
class RateRecommendationAgent(BaseAgent):
    ...
    def load_context(self) -> bool:
        ...
        try:
            possible_paths = [
                ...
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "data", "rate_recommendation.csv"
                ),
            ]
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        if not self.data_loaded:
            return {
                "error": "Rate data not loaded. Please check if rate_recommendation.csv exists."
            }
        ...
        print(
            f"Extracted - Origin: '{origin}', Destination: '{destination}', Container: '{container_type}'"
        )

        if not origin:
            return {
                "status": "no_data",
                "message": "Origin code not provided",
                "origin_code": None,
        ...
        if not destination:
            return {
                "status": "no_data",
                "message": "Destination code not provided",
                "origin_code": origin,
                "destination_code": None,
        ...
        if not container_type:
            return {
                "status": "no_data",
                "message": "Container type not provided",
                "origin_code": origin,
        ...
        # Raw price range recommendation and market data from CSV
        ...
        market_midlow = None
        market_midhigh = None
        market_range_formatted = None
        rate_quality = None
        contract_length = None
        day = None

        # Use exact match if available, otherwise the route-only fallback
            ...
            if market_avg_numeric is not None and pd.notna(market_avg_numeric):
                try:
                    ...
                    if avg_value > 0:
                        ...
                        # market_low/market_high kept for backward compatibility
                        market_low = str(int(range_low))
                        market_high = str(int(range_high))

                        print(
                            f"Market Range Calculated: {market_range_formatted} (based on average ${int(avg_value):,})"
                        )
                except (ValueError, TypeError) as e:
                    print(f"Could not calculate market range: {e}")

            # Fallback to the raw CSV values if the range calculation did not run
        ...
        return {
            ...
            "message": (
                "Rate data found" if indicative_rate else "No rate data available for this route"
            ),
            "origin_code": origin,
            "destination_code": destination,
            "container_type": container_type,
            ...
            "market_range": market_range_formatted,
...
def test_rate_recommendation_agent():
    ...
    test_cases = [
        ...
        {"origin": "AEAUH", "destination": "AUBNE", "container_type": "40HC"},
